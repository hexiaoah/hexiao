...
import {ENTRANCE_LOGIN} from "apiConfig";
...
function requestFactory(method) {
    ...
    return function request(url, ...args) {
        ...
        return realRequest(url, ...args).then(response => {
            ...
            // 授权失败
            if (code === -1 || result.errorCode == 'ERR_PUB200001'|| result.errorCode == 'ERR_PUB200002' || result.errorCode == 'ERR_PUB200003') {
                Modal.error({
                    title: "请注意",
                    content: "登录过期，请重新登录",
                    onOk() {
                        window.location.href = ENTRANCE_LOGIN + '?flag=timeout';
                    }
                });
                return ;
            }
