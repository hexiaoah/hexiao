            ...
            <Form :label-width="80">
                <FormItem label="加料分类">
                    <Select v-model="goodAttrAddFeedSubType.formData.name" style="width: 370px;margin: 0 10px"
                            placeholder="">
                        <Option v-for="item in feedTypeList" :value="item.value" :key="item.value+'_'+item.label">{{ item.label }}
                        </Option>
                    </Select>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...mapGetters({
                goodFeeds: "goods/formattedGoodfeed",
                goodFeedTypeList: 'goods/goodFeedTypeList'
            }),
            noData() {
                let self = this;
