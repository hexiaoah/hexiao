...
<template>
    <div>
        ...
        <BaseModal :show="pageState.modalShow" :header="pageState.modalTitle" okText="保存" @on-ok="ok"
            ...
            <Form :label-width="80">
                <FormItem label="单位名称">
                    <Input v-model="form.unitDesc" placeholder="" style="width: 370px;margin: 0 10px"
                           :disabled="pageState.modalType == 2"></Input>
                    <Verify :need="pageState.need" type="1" :value="form.unitDesc" :max="20"
                            @verify="verifyFormUnit"></Verify>
                </FormItem>
...
//                self.form.useUnitConversion = item.useUnitConversion
        ...
        created() {
            ...
            this.$store.dispatch('goods/getUnitList', self.pageParams)
            this.$store.dispatch('goods/getUnitConversion', self.pageParams)
        },
        components: {
            ButtonBar,
            BaseModal,
