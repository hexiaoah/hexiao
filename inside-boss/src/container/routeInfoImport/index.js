...
import Main from '../../components/routeInfoImport/main'
import InitData from './init'
import * as action from '../../action'
import * as bridge from '../../utils/bridge'
import styles from './style.css'

class routeInfoImportContainer extends Component {

    componentWillMount () {
        const query = bridge.getInfoAsQuery()
        const {dispatch, params} = this.props
        const data = InitData(params.pageType, query)
        dispatch(action.initRouteData(data))
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(routeInfoImportContainer)
