...
import {formatNumber} from '@src/container/visualConfig/views/design/utils/index'
import visualApi from '@src/api/visualConfigApi'
import CouponSelect from '@src/container/visualConfig/views/design/common/couponSelect'
...
export default class TmpEditor extends DesignEditor {
  ...
  bgChange = (item, index) => {
    ...
    this.configChang({backgroundColor: item.backgroundColor})
  }

  selectNum = ({key}) => {
    this.configChang({fetchLength: key})
  }
      ...
      <div className={`${prefix}-design-component-coupons-editor`}>
        ...
        <div className={style.componentCouponsEditor}>
          ...
          <div className={style.couponsRadioGroup}>
            <RadioGroup value={value.config.source} className={style.CouponsRelation} onChange={(e) => this.changeGroup('source', e)}>
