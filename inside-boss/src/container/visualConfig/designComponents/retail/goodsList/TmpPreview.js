...
export default class GoodsPreview extends PureComponent {
  ...
  componentWillReceiveProps(nextProps) {
      const nextPropsGoodsList = [...nextProps.value.config.goodsList]
      const oldPropsGoodsList = [...this.props.value.config.goodsList]
      if (nextPropsGoodsList.length != oldPropsGoodsList.length) {
          this.getSpecifyRetailGoodsList(nextPropsGoodsList)
      }
  }
  init = () => {
  ...
  previewType = () => {
    ...
    const { imgList } =  this.state
      ...
      return (
          imgList.length > 0
      ? imgList.map((item, i) => <div className={style[`rank${ItemClass[config.mode]}Img`]} key={i}>
                      <div className={style[`rank${ItemClass[config.mode]}List`]}>
                          <div className={cx(style.defultImg, 'desigeCove')} style={{ backgroundImage: `url(${item || constants.defaultGoodsImg})` }}>
      ...
      : ItemArr[config.mode].map((item, i) => <div className={style[`rank${ItemClass[config.mode]}Img`]} key={i}>
                      <div className={style[`rank${ItemClass[config.mode]}List`]}>
