...
import style from './GoodsPreview.css'
...
export default class GoodsPreview extends PureComponent {
  ...
  previewType = () => {
    ...
    const imglist = this.imgList(data, newGoodsList)

    return (
      imglist.length > 0 ?
      imglist.map((item, i) =>
        <div className={style[`rank${ItemClass[config.mode]}Img`]} key={i}>
      ...
      ItemArr[config.mode].map((item, i) =>
      <div className={style[`rank${ItemClass[config.mode]}Img`]} key={i}>
          <div className={style[`rank${ItemClass[config.mode]}List`]}>
            <div className={style.defultImg}>
