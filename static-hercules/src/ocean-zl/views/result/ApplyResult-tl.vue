<template>
    <div class="result-wrapper">
        <!--申请审核中-->
        <div class="applying-wrapper" v-if="openStatus === 'ALLIN_BLUE_SEA_AUDITING'">
            <i class="applying"></i>
            <p class="title">审核中</p>
        ...
        <!--申请失败-->
        <div class="fail-wrapper" v-if="openStatus === 'ALLIN_BLUE_SEA_AUDIT_FAIL'">
            <i class="fail"></i>
            <p class="title">审核失败</p>
            <div class="content">
                <p class="reason">{{auditMessage}}</p>
            </div>
            <span class="view-material result-btn" @click="applyAgain">重新申请</span>
        </div>
        <!--申请成功-->
        <div class="success-wrapper" v-if="openStatus === 'ALLIN_BLUE_AUDIT_SUCCESS'">
            <i class="success"></i>
...
export default {
  ...
  methods: {
    ...mapActions(['changeViewState', 'updataShopInfoTl']),
    // 重新申请
    applyAgain() {
      this.changeViewState('edit')
      this.$router.push({ path: '/input-tl' })
    },
    // 查看申请资料
    viewInfo() {
    ...
    getApplyStatus() {
      ...
      getApplyStatus(params)
        .then(data => {
          ...
          /**
           * auditStatus
           *    ALLIN_BLUE_SEA_AUDITING:审核中
           *    ALLIN_BLUE_AUDIT_SUCCESS:审核成功
           *    ALLIN_BLUE_SEA_AUDIT_FAIL:审核失败
           */
          let { auditStatus, auditMessage, allinBlueSeaInfo } = data
          if (auditStatus) {
            this.openStatus = auditStatus
            // this.openStatus = 'ALLIN_BLUE_SEA_AUDIT_FAIL'
            this.auditMessage = auditMessage
            this.updataShopInfoTl(allinBlueSeaInfo)
          }
        })
        .catch(e => {
          console.log(e)
