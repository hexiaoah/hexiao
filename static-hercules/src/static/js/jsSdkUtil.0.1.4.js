...
(function () {
  ...
  var APP_JS_SDK_URL = FILE_BASE_URL + "/js/FRWCardApp.0.0.6.js";
  ...
  // 预售域名下，或预售新人红包页面（便于项目、预发环境测试）用饭好约的公众号sdk鉴权接口
  var url = window.location.href || "";
  if (url.search("fanhaoyue.com") >= 0 || url.search("presellRedEnvelopes") >= 0 ) {
    WX_CONFIG_URL = SHARE_API_BASE_URL + "/share/v1/get_fan_jsapi_ticket";   // 饭好约公众号用
  }
  ...
  JsSdkUtil = {
    ...
    shareFailCallback: undefined,            // 分享失败后的回调, 默认无
    shareCancelCallback: undefined,            // 分享中断后的回调, 默认无
  ...
  //  获取 token
  function getToken() {
    var _DFMAP_COMMON_ = '_DFMAP_common_';
    var f_val;
    try {
      var key = 'token'
      var val = sessionStorage.getItem(_DFMAP_COMMON_ + key) || sessionStorage.getItem(_DFMAP_COMMON_ + "ytoken");  // 新人红包注册后的yToken
      if (val) {
        f_val = JSON.parse(val);
      } else {
        var error_msg = 'undefined_common_key:' + key;
        console.log(error_msg);
        window.DFAnalytics && window.DFAnalytics.fire('Er', error_msg, {});
        f_val = null;
      }
    } catch (e) {
      return null;
  ...
  function getEntityId() {
    var str = sessionStorage.getItem('_DFMAP_sid_current') || '';
    if (str.indexOf('_')) {
  ...
  function getSdkConfig(url) {
    ...
    if (jsApi) {
      ...
      ajax(url, { xtoken: getToken(), url: path }, function (resp) {
          wxQqInit(resp);
          setJsApi(page, resp)
        }, function (resp) {
  ...
  function bindShareData(shareInfo) {
    ...
    if (shareInfo.friend.shareUrl.match(/\#/) && shareInfo.friend.shareUrl.match(/\?/)) {
      var  router1 = shareInfo.friend.shareUrl.match(/\#\/[a-zA-Z]+/ig) ? shareInfo.friend.shareUrl.match(/\#\/[a-zA-Z]+/ig) :'';
      shareInfo.friend.shareUrl = shareInfo.friend.shareUrl.replace(/\#\/[a-zA-Z]+/ig, '') + router1;
    };
    if (shareInfo.moment.shareUrl.match(/\#/) && shareInfo.moment.shareUrl.match(/\?/)) {
      var  router2 = shareInfo.moment.shareUrl.match(/\#\/[a-zA-Z]+/ig) ? shareInfo.moment.shareUrl.match(/\#\/[a-zA-Z]+/ig) :'';
        shareInfo.moment.shareUrl = shareInfo.moment.shareUrl.replace(/\#\/[a-zA-Z]+/ig, '') + router2;
    };
    if (JsSdkUtil.client == 4) {    // 调用app分享接口
      jsSdk.share({
        ...
        wechatMiniExtension: friend.wechatMiniExtension,
        success: function (res) {
          // 用户确认分享后执行的回调函数
          if (JsSdkUtil.shareCallback) {
            JsSdkUtil.shareCallback(res)
          }
        },
        cancel: function (res) {
          if (JsSdkUtil.shareCancelCallback) {
            JsSdkUtil.shareCancelCallback(res)
          }
        },
        failer: function (res) {
          // 用户分享失败后执行的回调函数
          if (JsSdkUtil.shareFailCallback) {
            JsSdkUtil.shareFailCallback(res)
          }
        }
      });
          ...
          success: function (res) {
            if (JsSdkUtil.scanCallback) {
              ...
              if (JsSdkUtil.client == 7) {
                 ...
                 * 有问题联系@炒饭 和 @排骨
                 */
                window.location.href = API_BASE_URL + '/mini-app/jump/v1/scan?xtoken=' + token + '&url=' + encodeURIComponent(res.resultStr);
              } else {
