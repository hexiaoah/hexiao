...
<script>
    ...
    import {getBankSub} from 'src/ocean/config/api.js'
    ...
    export default {
        ...
        created() {
            let self = this;
            getBankSub(
                self.bankSub.bankCode,
                self.bankSub.bankCityId
            ).then(data => {
        ...
        methods: {
            ...
            bankClick(id) {
                this.modifyShopInfo({type: 'bankSubName', value: this.checkBankSub});
                this.modifyShopInfo({type: 'bankSubCode', value: id});
                this.$router.replace({
