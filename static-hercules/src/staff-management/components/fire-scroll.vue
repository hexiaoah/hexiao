...
export default {
  ...
  mounted () {
    let self = this
    setTimeout(self._initScroll, 2000)  // 这里要延时，因为这里的mounted是子控件的dom加载完成，需要等待父控件的dom加载完成，否则无法滑动
    // 当前页面有数据的情况下才重新计算宽度，否则去data里面去修改
    if (self.$refs.scrollwrapper.children[0].children.length) {
      self.$nextTick(self.refresh())
    }
  },
  watch: {
    data () {
