...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            changeInput($event) {
                const self = this;
                if(self.inputName === 'accountNumberNew') {  // 银行卡号--修改银行卡页面
                    self.modifyInputInfo({type: 'accountNumber', value: $event.target.value})
                } else if(self.inputName === 'businessAccountNumberNew') {  // 银行卡号--修改银行卡页面--企业商户
                    self.modifyInputInfo({type: 'businessAccountNumber', value: $event.target.value})
                } else {
                    self.modifyInputInfo({type: self.inputName, value: $event.target.value})
                }
            },
            clearInput($event) {
