...
export default {
    ...
    dateNewPicker: {
      ...
      handler: function() {
        if (this.dateNewPicker.showPicker) {
          ...
          if (
            this.dateNewPicker.pickerValue != null &&
            this.dateNewPicker.pickerValue != ''
          )
            this.currentValue = new Date(
              this.dateNewPicker.pickerValue.replace(/\./g, '-')
            )
          fixedBody()
        } else {
          looseBody()
  ...
  methods: {
    ...mapActions(['openDatePicker', 'modifyInputInfo']),
    open() {
      this.visible = true
    },
    close() {
